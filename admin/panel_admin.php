...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['borrar_user'])) {
    if (!validarCsrf()) {
        $mensaje = '<div class="alert alert-danger">Solicitud no válida. Vuelve a intentarlo.</div>';
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editar_user'])) {
    if (!validarCsrf()) {
        ...
        if ($id_editar > 0 && !empty($nuevo_nombre) && filter_var($nuevo_email, FILTER_VALIDATE_EMAIL) && in_array($nuevo_rol, ['admin', 'usuario'], true)) {
            ...
            $mensaje = '<div class="alert alert-danger">Datos de usuario no válidos.</div>';
        }
    }
}
...
$d_count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) AS c FROM historial_dominios WHERE analisis_completo = 1"))['c'] ?? 0;
$mal_count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) AS c FROM historial_dominios WHERE analisis_completo = 1 AND estado = 'maliciosa'"))['c'] ?? 0;
$sos_count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) AS c FROM historial_dominios WHERE analisis_completo = 1 AND estado = 'sospechosa'"))['c'] ?? 0;
$nc_count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) AS c FROM historial_dominios WHERE analisis_completo = 1 AND estado = 'no_concluyente'"))['c'] ?? 0;
...
$analisis = mysqli_query($conn, "
    SELECT h.id, h.token_publico, h.dominio, h.estado, h.fecha_escaneo, u.usuario
    FROM historial_dominios h
    LEFT JOIN usuarios u ON h.id_usuario = u.id
    WHERE h.analisis_completo = 1
    ORDER BY h.id DESC
    LIMIT 10
");
                    ...
                    <tbody>
                        <?php if ($analisis && mysqli_num_rows($analisis) > 0): ?>
                            <?php while ($row = mysqli_fetch_assoc($analisis)): ?>
                                ...
                                } elseif ($row['estado'] === 'no_concluyente') {
                                ...
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <?php $tokenInforme = asegurarTokenAnalisis($conn, (int)$row['id']); ?>
                                        <a href="../resultados/ver_resultado.php?r=<?= htmlspecialchars($tokenInforme, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-outline-primary btn-sm">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="5" class="text-center text-muted">No hay análisis registrados.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
